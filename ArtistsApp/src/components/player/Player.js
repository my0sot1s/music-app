...
class Player extends Component {
  constructor(props) {
    super(props);
    this.state = {
      song: this.props.navigation.state.params,
      playing: true,
      muted: false,
      shuffle: false,
      sliding: false,
      currentTime: 0,
      songIndex: this.props.navigation.state.params.songIndex | 0,
    };
  }
  togglePlay() {
  ...
  randomSongIndex() {
    let maxIndex = this.state.song.songs.length - 1;
  ...
  render() {
    let songPlaying = this.state.song.songs[this.state.songIndex];
    let songPercentage;
    ...
    let forwardButton;
    if (!this.state.shuffle && this.state.songIndex + 1 === this.state.song.songs.length) {
    ...
    let volumeButton;
    if (this.state.muted) {
      volumeButton = <Icon onPress={this.toggleVolume.bind(this)} style={styles.volume} name="ios-volume-mute" size={18} color="#fff" />;
    } else {
    ...
    let image = songPlaying.albumImage ? songPlaying.albumImage : this.state.song.artist.background;
    return (
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <Text style={styles.headerText}>
            {this.state.song.artist.name | this.state.song.album}
          </Text>
        </View>
